...
export default function ComingSoonSection() {
  const [data, setData] = React.useState<ComingSoonSectionProps[]>([]);
  ...
  const handleGetData = async () => {
    await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/sd-movies?${createQuery({
      ...
      const dataSend = res.data.data.reduce((acc: ComingSoonSectionProps[], item: any) => {
        acc.push(item);
        return acc;
      }, []);
  ...
  return (
    <section ref={ref} className='flex flex-col space-y-2'>
      <div className='flex items-center space-x-1'>
        <div className='font-bold text-white text-lg'>Coming Soon</div>
      </div>
            ...
            <CarouselItem key={idx} className='w-[166px] basis-2/5 sm:basis-1/4'>
              <div className='flex flex-col space-y-2'>
                ...
                <div className='space-y-1'>
                  ...
                  <div className='flex items-center space-x-1'>
                    <div className='text-gray-400 text-sm'>{`${moment(item.attributes.release_date).format('MMMM Do YYYY')}`}</div>
                  </div>
                </div>
              </div>
            </CarouselItem>
